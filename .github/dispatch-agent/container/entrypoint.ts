#!/usr/bin/env tsx
import { $ } from 'zx'
import { readFileSync, existsSync, mkdirSync, appendFileSync, rmSync } from 'fs'
...
async function runSpecialist(
  ...
  let fullPrompt = readFileSync(promptFile, 'utf-8') + '\n\n' + PROJECT_BLOCK + '\n\n' + issueContext
...
const gitignoreLines = existsSync(gitignorePath) ? readFileSync(gitignorePath, 'utf-8').split('\n') : []
if (!gitignoreLines.includes('.dispatch-agent/')) appendFileSync(gitignorePath, '.dispatch-agent/\n')
...
const CONFIG_PATH = '/workspace/dispatch-agent.config.json'
let projectConfig: ProjectConfig
try {
  projectConfig = JSON.parse(readFileSync(CONFIG_PATH, 'utf-8')) as ProjectConfig
} catch {
  console.error(`ERROR: dispatch-agent.config.json not found at ${CONFIG_PATH}`)
  console.error('Create this file in your repo root — see .github/dispatch-agent/dispatch.readme.md')
  process.exit(1)
}
...
// Stage 1: Implementer
// ---------------------------------------------------------------------------
const IMPLEMENTER_PROMPT = MODE === 'refine'
...
// ---------------------------------------------------------------------------
await runSpecialist('code-review', `${PROMPTS_DIR}prompt-code-review.md`, ISSUE_CONTEXT)
let crVerdict = readVerdict('/workspace/.dispatch-agent/code-review-verdict.txt')

if (crVerdict === 'FAIL') {
  logEvent('implementer-retry', 'start', 'code review failed — retrying implementer')
  const reviewContext = `## Code Review Findings (address these before proceeding)\n\n${
    readReport('/workspace/.dispatch-agent/code-review.md', '_No review report written._')
  }`
  await runSpecialist('implementer-retry', IMPLEMENTER_PROMPT, ISSUE_CONTEXT, reviewContext)
  await runSpecialist('code-review-retry', `${PROMPTS_DIR}prompt-code-review.md`, ISSUE_CONTEXT)
  ...
  if (crVerdict === 'FAIL') {
    ...
    const crReport = readReport('/workspace/.dispatch-agent/code-review.md', '_No review report written._')
    await $`gh issue comment ${ISSUE_NUMBER} --repo ${REPO} --body ${
      `## ❌ Dispatch Agent: Code Review failed after retry\n\n**Branch:** \`${BRANCH}\`\n\n${crReport}`
    }`
    process.exit(1)
  }
}
...
if (qaVerdict === 'FAIL') {
  ...
  const qaContext = `## QA Findings (address these before proceeding)\n\n${
    readReport('/workspace/.dispatch-agent/qa-report.md', '_No QA report written._')
  }`
  await runSpecialist('implementer-retry-2', IMPLEMENTER_PROMPT, ISSUE_CONTEXT, qaContext)
  await runSpecialist('code-review-retry-2', `${PROMPTS_DIR}prompt-code-review.md`, ISSUE_CONTEXT)
  ...
  if (qaVerdict === 'FAIL') {
    ...
    await $`gh issue edit ${ISSUE_NUMBER} --add-label hitl --repo ${REPO}`
    const qaReport = readReport('/workspace/.dispatch-agent/qa-report.md', '_No QA report written._')
    const crReport = readReport('/workspace/.dispatch-agent/code-review.md', '_No review report written._')
    await $`gh issue comment ${ISSUE_NUMBER} --repo ${REPO} --body ${
      `## ❌ Dispatch Agent: QA failed after retry\n\n**Branch:** \`${BRANCH}\`\n\n### QA Report\n\n${qaReport}\n\n### Code Review Report\n\n${crReport}`
    }`
    process.exit(1)
  }
}
...
const implM = metrics.get('implementer') ?? { sessions: 0, duration: 0, inputChars: 0 }
const crM   = metrics.get('code-review') ?? { sessions: 0, duration: 0, inputChars: 0 }
const qaM   = metrics.get('qa')          ?? { sessions: 0, duration: 0, inputChars: 0 }
const totalSes = implM.sessions + crM.sessions + qaM.sessions

const filesChanged = (await $`git diff --stat main..HEAD`.catch(() => ({ stdout: '' }))).stdout
  .trim().split('\n').pop() ?? 'unknown'
const commitCountRaw = (await $`git log main..HEAD --oneline`.catch(() => ({ stdout: '' }))).stdout.trim()
const commitCount = commitCountRaw ? commitCountRaw.split('\n').filter(Boolean).length : 0

const implReport = readReport('/workspace/.dispatch-agent/implementation-report.md', '_No implementation report written._')
const crReport   = readReport('/workspace/.dispatch-agent/code-review.md', '_No code review report written._')
const qaReport   = readReport('/workspace/.dispatch-agent/qa-report.md', '_No QA report written._')
...
// ---------------------------------------------------------------------------
const hasCommits = (await $`git log origin/main..HEAD --oneline`.catch(() => ({ stdout: '' }))).stdout.trim()
